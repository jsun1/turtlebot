...
class AStar(object):
    ...
    def __init__(self, statespace_lo, statespace_hi, x_init, x_goal, occupancy, resolution=1, padding=0):
    ...
    def is_free(self, x, verbose=False):
        """
        ...
        return self.occupancy.is_free(x, self.padding, verbose)
        ########## Code ends here ##########
    ...
    def solve(self):
        ...
        if not self.is_free(self.x_goal):
            rospy.loginfo("Goal position {} is not valid.".format(self.x_goal))
            return False
        while len(self.open_set) > 0:
            # rospy.loginfo("open_set length: {}".format(len(self.open_set)))
