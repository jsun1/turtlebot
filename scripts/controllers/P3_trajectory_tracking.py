...
class TrajectoryTracker:
    ...
    def compute_control(self, x, y, th, t):
        ...
        V_prev = max(V_PREV_THRES, abs(self.V_prev)) * (1 if self.V_prev >= 0 else -1)
        u_1 = xdd_d + self.kpx * (x_d - x) + self.kdx * (xd_d - V_prev * np.cos(th))
        u_2 = ydd_d + self.kpy * (y_d - y) + self.kdy * (yd_d - V_prev * np.sin(th))

        V = V_prev / (1.0 - (u_1 * np.cos(th) + u_2 * np.sin(th)) * dt)
        om = - u_1 * np.sin(th) + u_2 * np.cos(th)
        ########## Code ends here ##########
