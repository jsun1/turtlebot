...
class Supervisor:
    ...
    def __init__(self):
        ...
        self.test_publisher = rospy.Publisher('/test', String, queue_size=10)
    ...
    def delivery_request_callback(self, msg):
        ...
        if msg.data not in self.vendor_locations:
            rospy.loginfo("No food/vendor found")
        vendor_location = self.vendor_locations[msg.data]

        rospy.loginfo("Request received! goal_x: {}, goal_y:{}".format(vendor_location.x, vendor_location.y))

        self.x_g = vendor_location.x
        self.y_g = vendor_location.y
        #try:
        #    self.x_g = vendor_location.x
        #    self.y_g = vendor_location.y
    ...
    def loop(self):
		...
		self.mode = Mode.POSE
            ...
            #rospy.loginfo(">>>>>>>>>>>>>>>>>>Mode Nav")
            if self.close_to(self.x_g, self.y_g, self.theta_g):
                #rospy.loginfo(">>>>>>>>>>>>>>>>>>Too close")
                self.mode = Mode.IDLE
            else:
                #rospy.loginfo(">>>>>>>>>>>>>>>>>>Nav to pose")
                self.nav_to_pose()
