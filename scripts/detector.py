...
class Detector:
    ...
    def __init__(self):
        ...
        self.params = DetectorParams(verbose=True)
        ...
        self.whitelisted_labels = {"apple", "banana", "pizza", "orange", "stop sign"}
    ...
    def project_pixel_to_ray(self, u, v):
        ...
        ########## Code starts here ##########
        # TODO: Compute x, y, z.
        # Note from Jeffrey: we have 2 equations from the HW Figure 7, and we know
	# we need to return a unit vector (so that's 1 more equation) so we can solve
	# for x y and z. 
	ucfx = (u - self.cx) / self.fx
	vcfy = (v - self.cy) / self.fy
	z = np.sqrt(1.0 / (vcfy ** 2 + ucfx ** 2 + 1.0))
	x = ucfx * z
        y = vcfy * z
    ...
    def camera_callback(self, msg):
        ...
        if num > 0:
            ...
            for (box,sc,cl) in zip(boxes, scores, classes):
                ...
                if self.object_labels[cl] not in self.whitelisted_labels:
                    continue
                ...
                if self.object_labels[cl] in self.whitelisted_labels:
