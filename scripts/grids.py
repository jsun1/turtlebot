...
class StochOccupancyGrid2D(object):
    ...
    def is_free(self, state, padding_window_size=0, verbose=False):
        # combine the probabilities of each cell by assuming independence
        # of each estimation
        padded_window_size = self.window_size + padding_window_size
        # rospy.loginfo("checking is_free with window size {}".format(padded_window_size))
        p_total = 1.0
        lower = -int(round((padded_window_size-1)/2))
        upper = int(round((padded_window_size-1)/2))
        for dx in range(lower,upper+1):
            for dy in range(lower,upper+1):
                ...
                if verbose:
                    rospy.loginfo("Snapped to grid ({}, {})".format(x, y))
                grid_x = int((x - self.origin_x) / self.resolution)
                grid_y = int((y - self.origin_y) / self.resolution)
                if verbose:
                    rospy.loginfo("Grid position ({}, {})".format(grid_x, grid_y))
                if grid_y>0 and grid_x>0 and grid_x<self.width and grid_y<self.height:
                    ...
                    if verbose:
                        rospy.loginfo("p_total={}, threshold={}".format(p_total, self.thresh))
