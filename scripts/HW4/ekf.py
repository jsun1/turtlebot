...
class EkfLocalization(Ekf):
    ...
    def measurement_model(self, z_raw, Q_raw):
        ...
        H = np.array(H_list).reshape(2 * len(H_list), H_list[0].shape[1]) 
        ########## Code ends here ##########
    ...
    def compute_innovations(self, z_raw, Q_raw):
        ...
        hs_alpha = np.tile(hs[0], (I, 1))
        hs_r = np.tile(hs[1], (I, 1))
        vectorized_angle_diff = np.vectorize(angle_diff)
        V = np.dstack([vectorized_angle_diff(z_alpha, hs_alpha), z_r - hs_r])
        S = np.zeros((I, J, 2, 2))
        for i in range(I):
            for j in range(J):
                S[i][j] = np.linalg.inv(np.matmul(Hs[j], np.matmul(self.Sigma, Hs[j].T)) + Q_raw[i])

        D = np.matmul(np.matmul(np.expand_dims(V, axis = 2) ,S),np.expand_dims(V, axis = 3)).reshape(I, J)
        indicies = np.argmin(D, axis = 1)
        for i in range(len(indicies)):
            j = indicies[i]
            if abs(D[i][j]) < self.g**2:
                v_list.append(V[i][j])
...
class EkfSlam(Ekf):
    ...
    def transition_model(self, u, dt):
        ...
        g_sub, Gx_sub, Gu_sub = tb.compute_dynamics(self.x[0:3], u, dt)
        g[0:3] = g_sub
        Gx[0:3, 0:3] = Gx_sub
        Gu[0:3] = Gu_sub
        ########## Code ends here ##########
    ...
    def compute_predicted_measurements(self):
        ...
        for j in range(J):
            ...
            h, Hx[:, 0:3] = tb.transform_line_to_scanner_frame(np.array([alpha, r]), self.x[0:3], self.tf_base_to_camera, True)
            if j >= 2:
                x_cam = self.x[0] + self.tf_base_to_camera[0] * np.cos(self.x[2]) - self.tf_base_to_camera[1] * np.sin(self.x[2])
                y_cam = self.x[1] + self.tf_base_to_camera[0] * np.sin(self.x[2]) + self.tf_base_to_camera[1] * np.cos(self.x[2])
                Hx[:,idx_j:idx_j+2] = np.array([[1, 0],[x_cam*np.sin(alpha) - y_cam*np.cos(alpha), 1]])
            ########## Code ends here ##########
